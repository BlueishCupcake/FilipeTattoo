export const precautionsTexts = {
  title: "Cuidados com a tattoo",
  firstParagraph:
    "Após fazer a tatuagem, a pele fica muito sensivel e por isso é necessário ficar atento aos riscos de infecção. Logo após sair do estúdio, é importante manter a pele protegida com um plástico firme, por pelo menos 4 horas.",
  secondParagraph:
    "Após as 4 horas, remover o plástico para evitar que a pele fique muito úmida e quente. Também é importante lavar a tatuagem e aplicar um creme cicatrizante, para estimular a cicatrização mais rápida da pele.",
  listTitle: "O que não fazer nos primeiros dias",
  firstItem:
    "★ Não retirar as casquinhas que começam a se formar nos primeiros 4 dias após a tatuagem, pois ainda podem estar ligadas a camadas mais profundas da pele, onde a tinta ainda está se alojando;",
  secondItem:
    "★ Não coçar a tatuagem, pois pode agravar a irritação da pele e promover o surgimento de uma infecção devido à presença de bactérias debaixo das unhas;",
  thirdItem:
    "★ Não mergulhar a tatuagem em água, especialmente em locais públicos como piscinas ou praias, pois a grande parte das bactérias se desenvolve na água, aumentando o risco de infecção;",
  fourthItem:
    "★ Evitar pegar sol, porque os raios UV causam inflamação da pele e podem acabar deslocando as camadas de tinta da tatuagem, além de atrasar a cicatrização;",
  fifthItem:
    "★ Evitar passar creme em excesso na tatuagem, principalmente cremes com petróleo, pois criam uma barreira que impede a pele de respirar e cicatrizar corretamente;",
  sixthItem:
    "★ Evitar alimentos que prejudicam a cicatrização, como carne de porco, frituras e embutidos;",
  seventhItem:
    "★ Não usar roupa muito apertada, pois evita a respiração da pele e também pode acabar puxando as casquinhas de pele que ajudam na cicatrização;",
  eigthItem:
    "★ Evitar atividade física no local, a atividade física faz a pessoa produzir muito suor e isso pode atrapalhar a cicatrização e até mesmo danificar a tatuagem, o ideal é evitar por pelo menos uma semana.",
};
